...
module.exports.runSqlFile = function( file, callback ){
  async.waterfall([
...
module.exports.run = function( options, callback ){
  ...
  async.waterfall([
    this.runSqlFile.bind( null, options.setupPath )
  ...
  , function( results, result, done ){
      results = results.map( function( delta ){
  ...
  , function( deltas, done ){
      var onDelta = function( file, next ){
        ...
        fs.readFile( path.join( options.deltasDir, file ), function( error, result ){
          ...
          var version = file.slice( 0, -4 );

          this_.emit( 'before:delta', version, file, result );
          this_.emit( 'before:delta:' + version, file, result );

          debug( 'Running query', version );
          query( result.toString(), [], function( error ){
            if ( error ) return next( error );

            this_.emit( 'after:delta', version, file, result );
            this_.emit( 'after:delta:' + version, file, result );
